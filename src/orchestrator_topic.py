...
def main():
    ...
    result_df = pd.DataFrame(results).sort_values("topic_id")

    # --- 出力 ---
    header = (
        "Topic\t件数\t入力感情\t出力感情\t感情差分\t"
        + "\t".join(f"Persona({PERSONA_LABELS[p]})確率" for p in PERSONAS)
        + "\t"
        + "\t".join(f"Persona({PERSONA_LABELS[p]})比率" for p in PERSONAS)
    )
    print(header)
